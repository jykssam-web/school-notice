...
import { Send, CheckCircle, AlertCircle, Info, QrCode, Smartphone, Monitor, ChevronLeft, LayoutGrid, Globe, Lock, Key, LogOut, X, CheckCircle2, GraduationCap, Users, Trash2 } from "lucide-react";
import { motion, AnimatePresence } from "motion/react";
import { useParams, useNavigate } from "react-router-dom";
import { db } from "../lib/firebase";
import { doc, getDoc, setDoc, collection, getDocs, deleteDoc, query, orderBy, limit, onSnapshot, serverTimestamp } from "firebase/firestore";
...
export default function AdminView() {
  ...
  // Auto-fill title with teacher name on login
  useEffect(() => {
  ...
  useEffect(() => {
    // If we have school data but no user data, start at choice
  ...
  // If we're at the root /admin, we show the class selector
  const isLanding = !channelId;
  const activeChannel = channelId || "common";

  // Fetch initial history for the active channel
  useEffect(() => {
    if (!isLanding && schoolData?.id) {
      try {
        const q = query(
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
          ...
          console.error("History Subscription Error", e);
        });
        ...
        >
          ...
          {loginStage === "choice" && (
            <div className="space-y-6">
              ...
              <button 
                onClick={() => { setUserType("admin"); setLoginStage("user"); setLoginError(""); }}
              ...
              <button 
                onClick={() => { setUserType("teacher"); setLoginStage("user"); setLoginError(""); }}
            ...
            <div className="space-y-6">
              <div className="mb-8">
                ...
                <h2 className="text-2xl font-black">
                  {userType === 'admin' ? '관리자 확인' : '선생님 성함 입력'}
                </h2>
              </div>
              ...
              <form onSubmit={handleUserAuth} className="space-y-4">
                {userType === 'admin' ? (
                  ...
                  <div className="relative">
                    ...
                    <input 
                      type="text"
                      placeholder="선생님의 함자를 입력하세요 (예: 정쌤)"
  ...
  const isDevUrl = rootUrl.includes("-dev-");
  ...
  if (userData?.role === 'admin' && activeTab === 'members') {
    // Show members management even if isLanding is true
  } else if (isLanding) {
    return (
          ...
          {!selectedGrade ? (
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
                ...
                <button
                  onClick={() => handleClassToggle("common")}
                  className={`p-12 rounded-[48px] shadow-2xl hover:-translate-y-1 transition-all group flex flex-col items-center gap-4 relative ${
                    selectedChannels.includes("common") 
                      ? "ring-4 ring-blue-500 bg-blue-600/10 border-blue-500 text-white" 
                ...
                >
                  ...
                  <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all ${
                    selectedChannels.includes("common") ? "bg-white/10" : "bg-[#111111]/5"
                  }`}>
                    <Smartphone size={32} />
                  </div>
                  <span className="font-black text-2xl">전체 공지</span>
                  <span className={`text-xs font-bold uppercase tracking-widest ${
                    selectedChannels.includes("common") ? "text-blue-300" : "text-[#111111]/40"
                ...
                {currentGradeClasses.map((cls, idx) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`${
                        isSelected 
                          ? "ring-4 ring-blue-500 border-blue-500 bg-blue-600/10" 
                          : cls.isTotal 
                            ? "bg-white text-[#111111] border-transparent" 
                            : "bg-[#1A1A1A] text-white border-white/5"
                      } p-6 md:p-8 rounded-[32px] border shadow-2xl hover:shadow-blue-500/10 hover:border-blue-500/50 hover:-translate-y-1 transition-all group flex flex-col items-center gap-3 relative`}
                    >
                      {isSelected && (
                      ...
                      <div className={`${
                        cls.isTotal ? "bg-[#111111]/5" : "bg-white/5"
                      } w-12 h-12 rounded-2xl flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors`}>
  ...
  return (
    <div className="min-h-screen bg-[#111111] flex flex-col items-center p-6 md:p-12 font-sans text-white relative overflow-hidden">
      ...
      <div className="max-w-6xl w-full flex flex-col md:flex-row gap-8 relative z-10">
            ...
            <div className="flex justify-between items-start mt-4">
              <div>
                <h1 className="text-3xl font-black tracking-tight mb-2 uppercase">
                  {activeChannel === "common" 
                    ? "전체 공지 송신함" 
                    : activeChannel.includes(',')
                      ? `${activeChannel.split(',').length}개 반 동시 전송`
                      : activeChannel.endsWith("00")
                        ? `${activeChannel.charAt(0)}학년 전체 송신함`
          ...
          >
            <form onSubmit={handleSend} className="space-y-8">
              ...
              <button
                ...
                className={`w-full py-6 rounded-[24px] font-black text-xl flex items-center justify-center gap-3 transition-all active:scale-95 shadow-lg ${
                  status === "success"
                    ? "bg-emerald-500 text-white shadow-emerald-500/20"
                    : "bg-blue-600 text-white hover:bg-blue-700 shadow-blue-500/20 disabled:opacity-50"
                }`}
              >
                {status === "loading" ? "전송 중..." : status === "success" ? (
                  <><CheckCircle size={28} /> 전송 완료!</>
                ) : (
                  <><Send size={28} /> 공지사항 실시간 전송</>
                )}
              </button>
            </form>
          </motion.div>
        ...
        {/* Right Side: Setup & History */}
        <div className="flex-1 flex flex-col gap-6">
          
          {/* Display Address Bento Block */}
          <motion.div 
          ...
          >
            ...
            <div className="space-y-4">
              {isDevUrl && (
                <div className="bg-amber-500/5 border border-amber-500/20 p-4 rounded-2xl text-left mb-2">
                  <p className="text-[11px] font-black text-amber-500 uppercase mb-1">⚠️ 주의: 공유 주소 사용</p>
                  <p className="text-[10px] text-amber-200/60 leading-tight font-medium">
                    전자칠판에서 로그인 없이 바로 보려면 상단 <b>'Share'</b> 버튼을 통한 공유 주소를 사용해야 합니다.
                  </p>
                </div>
              )}
              ...
              <div className="flex flex-col gap-3 bg-white/5 p-4 rounded-2xl border border-white/5 text-left">
                <div className="flex flex-col gap-1">
                  ...
                  <code className="text-[13px] font-mono font-bold text-blue-400 break-all block py-1">
                    {displayUrl}
                  </code>
                </div>
                <button 
              ...
              <div className="flex items-start gap-3 p-2">
                ...
                <p className="text-[11px] text-neutral-500 leading-relaxed">
                  각 교실 칠판 브라우저에 위 주소를 입력하고 즐겨찾기해 두시면 편리하게 접속 가능합니다.
                </p>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="space-y-6 overflow-y-auto max-h-[350px] pr-2 custom-scrollbar">
              {history.length === 0 ? (
                <div className="text-center py-12 text-neutral-600 italic text-sm font-medium">
                  아직 전송된 공지가 없습니다.
                </div>
              ) : (
                history.map((item, i) => (
                  <div key={i} className="flex justify-between items-center group border-b border-white/5 pb-4 last:border-0 hover:bg-white/[0.02] -mx-2 px-2 rounded-xl transition-all">
                    <div className="min-w-0 flex-1">
                      <h3 className="text-[14px] font-bold text-neutral-300 group-hover:text-blue-500 transition-colors uppercase truncate">
                        {item.title}
                      </h3>
                      <p className="text-[10px] text-neutral-600 font-bold mt-0.5 uppercase tracking-wider">
                        {item.timestamp.toLocaleTimeString()} 전송
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="shrink-0 text-[10px] font-black px-2 py-0.5 bg-emerald-500/10 text-emerald-500 rounded-md uppercase border border-emerald-500/20">
                        ACTIVE
                      </span>
                      <button 
                        onClick={() => handleDelete(item.id, item.authorId)}
                        className="bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white p-2 rounded-xl transition-all shadow-lg active:scale-95"
                        title="삭제(해결 완료)"
                      >
                        <X size={14} />
                      </button>
        ...
        <div className="md:col-span-1 bg-[#1A1A1A] p-10 rounded-[40px] border border-white/5 shadow-2xl h-fit">
          <div className="flex items-center gap-3 mb-8">
            ...
            <form onSubmit={handleAddMember} className="space-y-4">
              ...
              <button 
                type="submit"
                disabled={memberLoading || !bulkNames.trim()}
        ...
        <div className="md:col-span-2 bg-[#1A1A1A] p-10 rounded-[40px] border border-white/5 shadow-2xl">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 h-[450px] overflow-y-auto pr-2 custom-scrollbar">
                {members.map((member) => (
                  <div key={member.name} className="bg-white/5 p-6 rounded-3xl border border-white/5 flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-black bg-blue-500/20 text-blue-500">
          ...
          <div className="mt-8 p-6 bg-white/5 rounded-[24px] border border-dashed border-white/10">
            <p className="text-[11px] text-neutral-500 leading-relaxed italic">
              * 각 학교의 관리자(정보부장님 등)는 소속 선생님들의 계정을 직접 관리할 수 있습니다. 
              선생님들의 개인정보 보호를 위해 최소한의 정보만 등록해 주세요.
            </p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="mt-20 py-8 border-t border-white/5 w-full flex flex-col items-center gap-4 relative z-10">
        ...
        <div className="flex items-center gap-6">
          <p className="text-xl font-black uppercase tracking-[0.2em] text-neutral-500">
            made by .공부하는 정쌤
          </p>
          <button 
